...
const DriverBadge = ({ driver }: DriverBadgeProps) => {
    const imageBackgroundColor: string = getTeamBackgroundColorDark(driver.team.id);
    const teamVarients: Record<string, string> = {
        alpine: 'hover:bg-alpine-dark/50',
        'aston-martin': 'hover:bg-aston-martin-dark/50',
        ferrari: 'hover:bg-ferrari-dark/50',
        haas: 'hover:bg-haas-dark/50',
        'kick-sauber': 'hover:bg-kick-sauber-dark/50',
        mclaren: 'hover:bg-mclaren-dark/50',
        mercedes: 'hover:bg-mercedes-dark/50',
        'racing-bulls': 'hover:bg-racing-bulls-dark/50',
        'red-bull': 'hover:bg-red-bull-dark/50',
        williams: 'hover:bg-williams-dark/50'
    };
    ...
    return (
        <>
            <Button
                ...
                variant='ghost'>
                <Link
                    className={`flex items-center justify-start w-full ${teamVarients[driver.team.id]} transition-colors duration-200`}
                    href={`/drivers/${driver.givenName}-${driver.familyName}`}>
                    <div className='flex justify-center gap-1'>
                        <span
                            className={`rounded-full overflow-hidden size-7 shadow-md ${imageBackgroundColor}`}>
                            <Image
